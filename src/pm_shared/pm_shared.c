...
qboolean PM_AddToTouched(pmtrace_t tr, vec3_t impactvelocity)
{
	...
	if (pmove->numtouch >= MAX_PHYSENTS)
		pmove->Con_DPrintf("Было закранута занадта шмат аб'ектаў!\n");
...
//
	for (i=0 ; i<3 ; i++)
	{
		...
		{
			pmove->Con_Printf ("PM  Атрымаў хуткасць NaN %i\n", i);
			pmove->velocity[i] = 0;
		}
		if (IS_NAN(pmove->origin[i]))
		{
			pmove->Con_Printf ("PM  Атрымаў паходжанне NaN на %i\n", i);
			pmove->origin[i] = 0;
		}
		...
		{
			pmove->Con_DPrintf ("PM  Занадта высокая хуткасць на %i\n", i);
			pmove->velocity[i] = pmove->movevars->maxvelocity;
		}
		else if (pmove->velocity[i] < -pmove->movevars->maxvelocity)
		{
			pmove->Con_DPrintf ("PM  Занадта нiзкая хуткасць на %i\n", i);
			pmove->velocity[i] = -pmove->movevars->maxvelocity;
		}
	}
}
...
void PM_PlayerMove ( qboolean server )
{
	...
	{
	default:
		pmove->Con_DPrintf("Фальшывы тып перамяшчэння гульца pmove %i на (%i) 0=cl 1=sv\n", pmove->movetype, pmove->server);
		break;
