...
bool AgModel::CheckModel(const char* szModelName)
{
  ...
  {
    char szMessage[256];
    sprintf(szMessage,"Сервер выконвае праверку мадэлі і, здаецца, %s пашкоджаны.\n",szModelName);
    AgLog(szMessage);
  ...
  {
    char szMessage[256];
    sprintf(szMessage,"Сервер выконвае праверку мадэлі, і %s несапраўдны. Ваша мадэль мае наступныя дыяпазоны: %.4f,%.4f,%.4f\n",szModelName,vBounds.x, vBounds.y, vBounds.z);
    AgLog(szMessage);
    sprintf(szMessage,"Сервер выконвае праверку мадэлі, і %s несапраўдны.\n",szModelName);
    ConsolePrint(szMessage);
#ifdef _DEBUG
		return true;
#else
