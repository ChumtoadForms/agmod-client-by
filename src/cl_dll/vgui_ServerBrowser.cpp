	...
	virtual Panel* getCellRenderer(int column,int row,bool columnSelected,bool rowSelected,bool cellSelected)
	{
		...
		else
		{
			if ( !row && !column )
			{
				...
				{
					m_pLabel->setText( "×àêàííå àäêàçó ñýðâåðà¢..." );
				}
				else
				{
					m_pLabel->setText( "Íàö³ñí³öå 'Àáíàâ³öü', êàá øóêàöü ñýðâåðû..." );
				}
			}
			else
			{
...
ServerBrowser::ServerBrowser(int x,int y,int wide,int tall) : CTransparentPanel( 64, x,y,wide,tall )
{
  ...
  pLabel[5] = new CSBLabel( "Description", "desc" );
	...
	_connectButton = new CommandButton( "Çëó÷ûöü", btnx, by, bw, bh );
	...
	_refreshButton = new CommandButton( "Àáíàâ³öü", btnx, by, bw, bh );
	...
	*/
	...
	_stopButton = new CommandButton( "Ñòîï", btnx, by, bw, bh );
  ...
  _NextPage = new CommandButton( "Áîëüø", btnx, by, bw, bh );
	...
	_cancelButton = new CommandButton( "Çà÷ûíiöü", btnx, by, bw, bh );
