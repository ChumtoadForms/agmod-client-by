...
CSchemeManager::CSchemeManager( int xRes, int yRes )
{
	...
	while ( strlen(token) > 0 && (currentScheme < numTmpSchemes) )
	{
		...
		if ( stricmp( token, "=" ) )
		{
			if ( currentScheme < 0 )
			{
				gEngfuncs.Con_Printf( "памылка разбору тэкставага файла схемы шрыфтоў пры пачатку файла - чакаецца '=', знойдзена'%s''\n", token );
			}
			else
			{
				gEngfuncs.Con_Printf( "памылка разбору тэкставага файла схемы шрыфтоў пры схеме - чакаецца '=', знойдзена'%s''\n", tmpSchemes[currentScheme].schemeName, token );
			}
			break;
		}
		...
		if ( !pScheme )
		{
			gEngfuncs.Con_Printf( "тэкставы файл схемы шрыфтоў ПАВІНЕН пачынацца з 'SchemeName'\n");
			break;
		}
	...
	for ( int i = 0; i < m_iNumSchemes; i++ )
	{
		...
		if ( !m_pSchemeList[i].font )
		{
			...
			if(g_CV_BitmapFonts && g_CV_BitmapFonts->value)
			{
				...
				if(!pFontData)
					gEngfuncs.Con_Printf("Адсутнічае растравы шрыфт: %s\n", fontFilename);
			}
