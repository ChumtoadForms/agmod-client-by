...
bool CIrcSession::Connect(const CIrcSessionInfo& info)
{
    ...
    m_identServer.Start(info.sUserID.c_str());
		...
		if( !m_socket.Create() )
			throw "Не ўдалося стварыць сокет!";

		InetAddr addr(info.sServer.c_str(), (WORD)info.iPort);
		if( !m_socket.Connect(addr) )
		{
			m_socket.Close();
			throw "Не ўдалося падключыць сокет!";
		}
...
{
	static const char* lpszFmt = "Ваш хост: %[^ \x5b,], версiя: %s";
...
{
	static const char* lpszFmt = "спрабуем %[^ ,], порт %d";
