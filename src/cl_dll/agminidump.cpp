	...
	if (hDll)
	{
		...
		if (pDump)
		{
			...
			if (hFile!=INVALID_HANDLE_VALUE)
			{
				...
				if (bOK)
				{
					sprintf( szScratch, "Захаваны файл дампа ў '%s'", szMiniDump );
					szResult = szScratch;
					//retval = EXCEPTION_EXECUTE_HANDLER;
				}
				else
				{
					sprintf( szScratch, "Не ўдалося захаваць файл дампа ў '%s' (error %d)", szMiniDump, GetLastError() );
					szResult = szScratch;
				}
				::CloseHandle(hFile);
			}
			...
			{
				sprintf( szScratch, "Не ўдалося стварыць файл дампа '%s' (error %d)", szMiniDump, GetLastError() );
				szResult = szScratch;
			}
		}
		else
		{
			szResult = "DBGHELP.DLL зусiм стары";
		}
	}
	else
	{
		szResult = "DBGHELP.DLL не знайдзены";
	}
