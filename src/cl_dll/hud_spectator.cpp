...
void SpectatorMode(void)
{
	...
	{
		gEngfuncs.Con_Printf( "выкарыстанне: spec_mode <Асноўны рэжым> [<Рэжым урэзкі>]\n" );
		return;
	}
...
void SpectatorMenu( void )
{
	if ( gEngfuncs.Cmd_Argc() <= 1 )
	{
		gEngfuncs.Con_Printf( "выкарыстанне: spec_menu <0|1>\n" );
		return;
	}
...
int CHudSpectator::Init()
{
	...
	{
		gEngfuncs.Con_Printf("ПАМЫЛКА! Не ўдалося зарэгістраваць усе зменныя назіральніка.\n");
		return 0;
	}
...
int UTIL_FindEntityInMap(char * name, float * origin, float * angle)
{
	...
	while (data)
	{
		...
		if (!data)
		{
			gEngfuncs.Con_DPrintf("UTIL_FindEntityInMap: EOF без закрывальнай дужкі\n");
			return 0;
		}

		if (token[0] != '{')
		{
			gEngfuncs.Con_DPrintf("UTIL_FindEntityInMap: чакаецца {\n");
			return 0;
		}
		...
		{	
			...
			if (token[0] == '}')
			{
				gEngfuncs.Con_DPrintf("UTIL_FindEntityInMap: закрываючая дужка без дадзеных");
				return 0;
			}
...
void CHudSpectator::DirectorMessage( int iSize, void *pbuf )
{
	...
	{ 
		...
		default			:	gEngfuncs.Con_DPrintf("CHudSpectator::DirectorMessage: невядомая каманда %i.\n", cmd );
	}
}
...
void CHudSpectator::FindNextPlayer(bool bReverse)
{
	...
	if ( !g_iUser2 )
	{
		gEngfuncs.Con_DPrintf( "Няма мішэняў для назіральнікаў.\n" );
...
void CHudSpectator::SetModes(int iNewMainMode, int iNewInsetMode)
{
	...
	{
		gEngfuncs.Con_Printf("Няправільны рэжым назіральніка.\n");
		return;
	}
...
bool CHudSpectator::ParseOverviewFile( )
{
	...
	if (!pfile)
	{
		gEngfuncs.Con_Printf("Немагчыма адкрыць файл %s. Выкарыстаю стандартныя значэння для рэжыму агляду.\n", filename );
		return false;
	}
	...
	while (true)
	{
		...
		if ( !stricmp( token, "global" ) )
		{
			...
			if ( stricmp( token, "{" ) ) 
			{
				gEngfuncs.Con_Printf("Памылка аналізу файла агляду %s. (чакаецца { )\n", filename );
				return false;
			}
