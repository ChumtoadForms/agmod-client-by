...
bool AgCRC32EnforceFile(char* pszFile, WORD32 w32CheckSum)
{
  ...
  {
		char szMessage[256];
		sprintf(szMessage,"Выканана праверка файла, і %s альбо пашкоджаны, альбо зменены. Запусціце scandisk і пераўсталюйце файл.\n", pszFile);
		AgLog(szMessage);
		ConsolePrint(szMessage);

    ServerCmd( "say <AG Mod> Адключана з-за выкарыстання несапраўднага файла.\n" );
