...
int AgHudSettings::Draw(float fTime)
{
  ...
  sprintf(szText,"Адрэналiнавы Гулец %s",m_szVersion);
  ...
  gHUD.DrawHudString(ScreenWidth/20 , gHUD.m_scrinfo.iCharHeight*3, ScreenWidth,"Martin Webrant, rogender",r,g,b);
  ...
  sprintf(szText,"Ліміт часу: %d",m_iTimeLimit);
  gHUD.DrawHudString(x, y,ScreenWidth,szText,r,g,b);
  y += gHUD.m_scrinfo.iCharHeight;

  sprintf(szText,"Ліміт фрагаў: %d",m_iFragLimit);
  gHUD.DrawHudString(x, y,ScreenWidth,szText,r,g,b);
  y += gHUD.m_scrinfo.iCharHeight;

  sprintf(szText,"Дружны агонь: %s",m_iFriendlyFire ? "On" : "Off");
  gHUD.DrawHudString(x, y,ScreenWidth,szText,r,g,b);
  y += gHUD.m_scrinfo.iCharHeight;
  
  sprintf(szText,"Застава зброі: %s",m_iWeaponstay ? "On" : "Off");
  gHUD.DrawHudString(x, y,ScreenWidth,szText,r,g,b);
  y += gHUD.m_scrinfo.iCharHeight;
  ...
  {
	sprintf(szText,"Стрэл у галаву: %sX (3)",m_szHeadShot);
	gHUD.DrawHudString(x, y,ScreenWidth,szText,r,g,b);
	y += gHUD.m_scrinfo.iCharHeight;
  }
  if (strlen(m_szBlastRadius))
  {
	sprintf(szText,"Радыус выбуху: %sX (1)",m_szBlastRadius);
	gHUD.DrawHudString(x, y,ScreenWidth,szText,r,g,b);
	y += gHUD.m_scrinfo.iCharHeight;
  }  
  if (g_iMatch)
  {
    strcpy(szText,"Матч ідзе!");
