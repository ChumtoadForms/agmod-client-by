...
#ifdef _DEBUG
  char szTime[64];
  sprintf(szTime,"Чыт праверку ўзяў %dms\n",int((GetTickCount() - dwTime)));
  ConsolePrint(szTime);
  AgLog(szTime);
#endif  
...
void AgHudGlobal::UserCmd_LoadAuthID()
{
  ...
  {
		char szMessage[256];
		sprintf(szMessage, "Не атрымалася загрузіць файл %s\n", pszFileName);
  ...
  char szCount[64];
  sprintf(szCount,"Загружаныя ідэнтыфікатары аўтарызацыі - %d\n",(int)s_mapRealName.size());
  ConsolePrint(szCount);
}

void AgHudGlobal::UserCmd_UnloadAuthID()
{
  ConsolePrint("Выгрузіў усе аўтэнтыфікатары\n");
  s_mapRealName.clear();
}
