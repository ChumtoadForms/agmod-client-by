  ...
  for (int i = 0; i < iElements; i++)
  {
      ...
      ||dValue > (pvarArray[i].dMax + 0.0001))
    {
      if (pvarArray[i].bAllowChangeAtInit)
      {
        ...
        char szMessage[256];
        sprintf(szMessage,"Сервер выконвае зменныя, і \"%s\" быў аўтаматычна скінуты да значэння HL па змаўчанні %f.\n",pvarArray[i].szName,pvarArray[i].dDefault);        
      ...
      else
      {
        ...
        char szMessage[256];
        sprintf(szMessage,"Сервер выконвае зменныя, і \"%s\" павінен быць паміж %f і %f. Ваша зменная ўстаноўлена ў %f.\n",pvarArray[i].szName,pvarArray[i].dMin,pvarArray[i].dMax,dValue);        
        ConsolePrint(szMessage);
        AgLog(szMessage);
        return false;
      }
  ...
  for (int i = 0; i < iElements; i++)
  {
      ...
      ||dValue > (pvarArray[i].dMax + 0.0001))
    {
      ...
      if (pvarArray[i].bKeepSame && pvarArray[i].bAllowChangeAtInit)
        sprintf(szMessage,"Сервер забяспечвае выкананне зменных, і \"%s\" павінен быць аднолькавым падчас гульні\n",pvarArray[i].szName);
      else
        sprintf(szMessage,"Сервер выконвае зменныя, і \"%s\" павінен быць паміж %f і %f.\n",pvarArray[i].szName,pvarArray[i].dMin,pvarArray[i].dMax);
      ConsolePrint(szMessage);
      ...
      sprintf(szMessage,"say <AG Mod> Папярэджанне за выкарыстанне зменных са значэннем %of (%d парушэнне(і)",pvarArray[i].szName,dValue,iViolations);
      ServerCmd(szMessage);
    }
  }
}
...
bool AgVariableChecker::Init()
{
  ...
  static char szMessage[] = "Збой ініцыялізацыі зменнай, выхожу.\n";
  ConsolePrint(szMessage);
  AgLog(szMessage);

  ServerCmd( "say <AG Mod> Адключана з-за выкарыстання несапраўднай зменнай.\n" );
...
bool AgVariableChecker::Check()
{
  ...
  {
    static char szMessageServer[] = "say <AG Mod> Адключаны за паўторныя парушэнні cvar\n";
    static char szMessage[] = "Чыт-праверка: адключана з-за паўторных парушэнняў cvar\n";
